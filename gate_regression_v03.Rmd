...
flowdf <- read.csv("armco_gate18.csv", header = TRUE, check.names = FALSE, fileEncoding="UTF-8-BOM")

ngates <- 1 #length(flowdf)
```
...
# model formula
formulastring <- list()
formulastring[[1]] <- ( a + b * flowdf$G + c * flowdf$G ^ 2 + d * flowdf$G ^ 3 + e * log( flowdf$H ) ) / ( 1 + aa * flowdf$G +  bb * log( flowdf$H ) + cc * ( log( flowdf$H ) ) ^ 2 + dd * ( log( flowdf$H ) ) ^ 3 )
...
for(di in 1:ngates){
  for (m in 1:nformulas){
    png(paste0("outputs/obspreplots/ovp_d", di, "_m", m, ".png"), width=3.25, height=2.85, units="in", pointsize=8, res=1200)
      plot(flowdfl[[di]]$FLOW, modelpred[[di]][[m]], ylab="Observed Flow", xlab="Predicted Flow", pch=19)
      
      # adj line for the perfect fit
      abline(0,1, col="grey80", lty=2, lwd=2)
      
      # adj line for linear fit
      lmmod <- lm(modelpred[[di]][[m]]~flowdfl[[di]]$FLOW)
...
# I have results from this section, but I would like to modify them. The current results are changing the cv and a variable sd (tsd, gsd, or hsd) at the same time. I would prefer the code to change the variable sd without changing the cv. I could manually choose a different cv to look at different scenarios. Or it could be nested loops. But what I wanted was to see the impacts of those factors separately, not changing them together. If they were nested loops it would be a ton of plot results (9 steps for the cv * 24 scenarios for the variables = over 200 plots), so maybe better to avoid that?
