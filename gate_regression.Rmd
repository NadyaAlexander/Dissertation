...
# plot min, max, mean, ... 
library(ggplot2)
png('outputs/mean_toymodel_flowerror.png', width=6.5, height=3, units="in", pointsize=8, res=300)
...
png('outputs/min_toymodel_flowerror.png', width=6.5, height=3, units="in", pointsize=8, res=300) 
...
png('outputs/max_toymodel_flowerror.png', width=6.5, height=3, units="in", pointsize=8, res=300)
...
# first let's make a cumulative vector fro each lateral
...
network_hsd_gcid <- network_gatetype_gcid
network_hsd_gcid[,] <- NA
for(i in 1:maxlaterals){
  for(j in 1:maxgates){
    network_hsd_gcid[j,i] <- 0.01*network_numgates_gcid[j,i]+0.1
  }
}

networkresult_gcid <- replicate(n=maxlaterals, expr=list())
# NOTE: only changing the uncertainty in upstream level based on location in the network
for(l in 1:maxlaterals){
  for(g in 1:maxgates){
    dd <- network_gatetype_gcid[g, l]
    hsdn <- network_hsd_gcid[g, l]
    networkresult_gcid[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.01, optflowerrorp=0.12, gsd=0.1, hsd=hsdn, tsd=0.25)
  }
}
