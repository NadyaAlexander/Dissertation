...
flowdf[[1]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE)
flowdf[[2]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE)
flowdf[[3]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE)
flowdf[[4]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE)
...
# plot min, max, mean, ... 
library(ggplot2)
png('outputs/mean_toymodel_flowerror.png', width=6.5, height=4, units="in", pointsize=8, res=300)
...
png('outputs/min_toymodel_flowerror.png', width=6.5, height=4, units="in", pointsize=8, res=300) 
...
png('outputs/max_toymodel_flowerror.png', width=6.5, height=4, units="in", pointsize=8, res=300)
...
png('outputs/mean_noncompliancepercentage_toymodel_flowerror.png', width=6.5, height=4, units="in", pointsize=8, res=300)
...
colnames(network_gatetype_gcid) <- paste0("L", gcid$LATID)
...
# CASEI: Well-behaved lateral, LATID=48-1
mean(latlvl[latlvl$LATID=="48-1", "VALUE"])
min(latlvl[latlvl$LATID=="48-1", "VALUE"])
max(latlvl[latlvl$LATID=="48-1", "VALUE"])

# CASEII: NOT Well-behaved lateral, LATID=22-1
mean(latlvl[latlvl$LATID=="22-1", "VALUE"])
min(latlvl[latlvl$LATID=="22-1", "VALUE"])
max(latlvl[latlvl$LATID=="22-1", "VALUE"])

# disregard LATID= 21-4 for now
network_hsd_gcid_case1 <- network_hsd_gcid_case2 <- network_gatetype_gcid
network_hsd_gcid_case1[,] <- NA
for(i in 1:maxlaterals){
  for(j in 1:maxgates){ # this 0.01 value will have to be tweaked after running this a few times. 
    network_hsd_gcid_case1[j,i] <- 0.01*network_numgates_gcid[j,i] + mean(latlvl[latlvl$LATID=="48-1", "VALUE"])
    network_hsd_gcid_case2[j,i] <- 0.01*network_numgates_gcid[j,i] + mean(latlvl[latlvl$LATID=="22-1", "VALUE"])
  }
}

networkresult_gcid_case1 <- networkresult_gcid_case2 <- replicate(n=maxlaterals, expr=list())
# NOTE: only changing the uncertainty in upstream level based on location in the network
for(l in 1:maxlaterals){
  for(g in 1:maxgates){
    ...
    networkresult_gcid_case1[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.02, optflowerrorp=0.06, gsd=0.1, hsd=hsdn, tsd=0.25)
    # CASEII
    hsdn <- network_hsd_gcid_case2[g, l]
    networkresult_gcid_case2[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.02, optflowerrorp=0.06, gsd=0.1, hsd=hsdn, tsd=0.25)
  }
}

library(ggplot2)
network_postprocessing <- function(data){
  # first let's put it in a dataframe
  ...
  # plot min, max, mean, ... 
  png('outputs/mean_gcid_flowerror.png', width=20, height=8, units="in", pointsize=8, res=300)
    ggplot(gcid_abs_mean_flowerror, aes(x=LATNUM, y=GATENUM, size=MEANABSFLOWERROR, fill=MEANABSFLOWERROR)) +
      geom_point(shape = 21)+
      scale_fill_distiller(palette = "Spectral")+
      labs(fill = "Mean \nAbsolute \nFlow Error \n(cfs)", size="Mean \nAbsolute \nFlow Error \n(cfs)")+
      xlab("Lateral No.") + 
  ...
  png('outputs/min_gcid_flowerror.png', width=20, height=8, units="in", pointsize=8, res=300) 
    ggplot(gcid_min_flowerror, aes(x=LATNUM, y=GATENUM, size=MINABSFLOWERROR, fill=MINABSFLOWERROR)) +
      geom_point(shape = 21)+
      scale_fill_distiller(palette = "Spectral")+
      labs(fill = "Minimum \nAbsolute \nFlow Error \n(cfs)", size="Minimum \nAbsolute \nFlow Error \n(cfs)")+
      xlab("Lateral No.") + 
  ...
  png('outputs/max_gcid_flowerror.png', width=20, height=8, units="in", pointsize=8, res=300)
    ggplot(gcid_max_flowerror, aes(x=LATNUM, y=GATENUM, size=MAXABSFLOWERROR, fill=MAXABSFLOWERROR)) +
      geom_point(shape = 21)+
      scale_fill_distiller(palette = "Spectral")+
      labs(fill = "Maximum \nAbsolute \nFlow Error \n(cfs)", size="Maximum \nAbsolute \nFlow Error \n(cfs)")+
      xlab("Lateral No.") + 
  ...
  png('outputs/mean_noncompliancepercentage_gcid_flowerror.png', width=20, height=8, units="in", pointsize=8, res=300)
    ggplot(gcid_abs_mean_errorcondition, aes(x=LATNUM, y=GATENUM, size=ERRORCONDITIONNOTMET, fill=ERRORCONDITIONNOTMET)) +
      geom_point(shape = 21)+
      scale_fill_distiller(palette = "Spectral")+
      labs(fill = "Percentage of \nNon-Compliance \n(N=100)", size="Percentage of \nNon-Compliance \n(N=100)")+
      xlab("Lateral No.") + 
...
network_postprocessing(networkresult_gcid_case1)
network_postprocessing(networkresult_gcid_case2)



# first let's put it in a dataframe
gcid_results <- data.frame(do.call(rbind, unlist(networkresult_gcid, recursive=FALSE, use.names = TRUE)))
gcid_results$LATNUM <- rep(1:maxlaterals, times=1, each=maxgates*nsim) # for plotting on the x axis
gcid_results$GATENUM <- rep(1:maxgates, times=maxlaterals, each=nsim) # for plotting on the y axis
gcid_results$LATID <- colnames(network_gatetype_gcid)[gcid_results$LATNUM]
gcid_results$GATEID <- rownames(network_gatetype_gcid)[gcid_results$GATENUM]
...
# 8.0 Uncertainty at the Lateral Level
```{r lateral_uncertainty}
# lateral head 
# the device 
# the volume that goes through it 
# the accuracy 
# This should give us the volumetric uncertainty

# total error in the lateral

# total error from the lateral heading measurements

# case 3 is a wave is coming down and making more error.
