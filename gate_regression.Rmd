...
for(d in 1:4){
  for (m in 1:8){
    ...
    modelpred[[d]][[m]] <- predict(modelsim, data=flowdfl, type="response")
    
    # fit an lm to obs vs. pred
    ...
    if(lmmod$coefficients[2]>1){
      lmmod <- lm(flowdf_long$LOGFLOW~model_pred)
    }
    
    modelstats_b[[d]][[m]] <- lmmod$coefficients[2]
    modelstats_r2[[d]][[m]] <- modelfits[[d]][[m]]$rsquared
  }
}
```
...
# 4.0 Model Plots
```{r model_plots}
# make sure to change this to be bR2, it's just b right now

for(d in 1:4){
  for (m in 1:8){
    png(paste0("outputs/obspreplots/ovp_d", d, "_m", m), width=3.25, height=2.85, units="in", pointsize=8, res=1200)
    print(plot(flowdfl[[d]]$LOGFLOW, modelpred[[d]][[m]], main="Predicted vs. Observed", ylab="Predicted Log Flow", xlab="Observed Log Flow"))
    # add an abline for the perfect fit
    ...
    mtext(paste("Y =", round(lmline$coefficients[2],3), "X +", round(lmmod$coefficients[1],0)), side=3, line=1, cex=0.8, adj=0.1)
    if(lmmod$coefficients[2]>1){
      lmmod <- lm(flowdfl[[d]]$LOGFLOW~modelpred[[d]][[m]])
    }
    mtext(paste("Model R2:", round(lmmod$coefficients[2],3)), side=3, line=0.3, cex=0.8, adj=0.1)
    dev.off()
  }
}
