...
for(d in 1:ngates){
  for (m in 1:nformulas){
    ...
    modelstats[[d]][[m]] <- gof(modelpred[[d]][[m]], flowdfl[[d]]$LOGFLOW)
...
write.csv(modelcsv, "outputs/gatemodeling.csv", row.names=FALSE)
```
...
for(d in 1:ngates){
  for (m in 1:nformulas){
    modelcsv$COEF_A[i] <- modelcoef[[d]][[m]]["A"]
    modelcsv$COEF_B[i] <- modelcoef[[d]][[m]]["B"]
    modelcsv$COEF_C[i] <- modelcoef[[d]][[m]]["C"]
    modelcsv$COEF_D[i] <- modelcoef[[d]][[m]]["D"]
    modelcsv$COEF_E[i] <- modelcoef[[d]][[m]]["E"]
    modelcsv$COEF_FF[i] <- modelcoef[[d]][[m]]["FF"]
    modelcsv$ME[i] <- t(modelstats[[d]][[m]])[1]
    modelcsv$MAE[i] <- t(modelstats[[d]][[m]])[2]
    modelcsv$MSE[i] <- t(modelstats[[d]][[m]])[3]
    modelcsv$RMSE[i] <- t(modelstats[[d]][[m]])[4]
    modelcsv$NRMSE[i] <- t(modelstats[[d]][[m]])[5]
    modelcsv$PBIAS[i] <- t(modelstats[[d]][[m]])[6]
    modelcsv$RSR[i] <- t(modelstats[[d]][[m]])[7]
    modelcsv$rSD[i] <- t(modelstats[[d]][[m]])[8]
    modelcsv$NSE[i] <- t(modelstats[[d]][[m]])[9]
    modelcsv$mNSE[i] <- t(modelstats[[d]][[m]])[10]
    modelcsv$rNSE[i] <- t(modelstats[[d]][[m]])[11]
    modelcsv$d[i] <- t(modelstats[[d]][[m]])[12]
    modelcsv$md[i] <- t(modelstats[[d]][[m]])[13]
    modelcsv$rd[i] <- t(modelstats[[d]][[m]])[14]
    modelcsv$cp[i] <- t(modelstats[[d]][[m]])[15]
    modelcsv$r[i] <- t(modelstats[[d]][[m]])[16]
    modelcsv$R2[i] <- t(modelstats[[d]][[m]])[17]
    modelcsv$bR2[i] <- t(modelstats[[d]][[m]])[18]
    modelcsv$KGE[i] <- t(modelstats[[d]][[m]])[19]
    modelcsv$VE[i] <- t(modelstats[[d]][[m]])[20]
    i <- i+1
  }
}
write.csv(modelcsv, "outputs/gatemodeling2.csv", row.names=FALSE)
```
...
for(d in 1:ngates){
  for (m in 1:nformulas){
    png(paste0("outputs/obspreplots/ovp_d", d, "_m", m, ".png"), width=3.25, height=2.85, units="in", pointsize=8, res=1200)
      plot(flowdfl[[d]]$LOGFLOW, modelpred[[d]][[m]], ylab="Observed Log Flow", xlab="Predicted Log Flow", pch=19)
      ...
      mtext(paste("Model bR2:", round(t(modelstats[[d]][[m]])[18],3)), side=3, line=0.3, cex=0.8, adj=0.1)
    dev.off()
  }
}
```

# 5.0 Uncertainty from Flow Table Equation -- Monte Carlo Sim
...
montecarlosim <- function(coefofvar, dataindex){
  ...
  atrue <- modelcoef[[d]][[5]]["A"] # assume the mean is the true value
  astdv <- coefofvar*modelcoef[[d]][[5]]["A"]
  anorm <- rnorm(nsim, modelcoef[[d]][[5]]["A"], astdv) # samples around true value, creating error
  
  btrue <- modelcoef[[d]][[5]]["B"]
  bstdv <- coefofvar*modelcoef[[d]][[5]]["B"]
  bnorm <- rnorm(nsim, modelcoef[[d]][[5]]["B"], bstdv)
  
  ctrue <- modelcoef[[d]][[5]]["C"]
  cstdv <- coefofvar*modelcoef[[d]][[5]]["C"]
  cnorm <- rnorm(nsim, modelcoef[[d]][[5]]["C"], cstdv)
  
  fftrue <- modelcoef[[d]][[5]]["FF"]
...
for(d in 1:ngates){
  for(c in 1:length(cvrange)){
    cv <- cvrange[c]
    optresult[[d]][[c]] <- equationerrorsim(dataindex=d, coefofvar=cv, optflowerrorp=0.06)
  }
}

for(c in 1:length(cvrange)){
  i <- cvrange[c]
  png(paste0('outputs/cloud/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
  ...
  mtext(paste0("Coefficient of Variation: ", i), side=3, line=0)
...
png('outputs/optimization_function2.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
...
opflowsim <- data.frame(matrix(NA, nrow=nsim, ncol=ngates))
opvarflow <- replicate(n=ngates, expr=list())
opvolsim <- data.frame(matrix(NA, nrow=nsim, ncol=ngates))
...
for(d in 1:ngates){
  ...
  opvarflow[[d]] <- sd(opflowsim[,d], na.rm=TRUE)^2
  ...
  for(j in 1:nsim){
    opvolsim[j,d] <- (anorm[j]*gnorm[j]^2 + bnorm[j]*gnorm[j] + cnorm[j])*(hnorm[j]^ffnorm[j])*tnorm[j]
  }
  opvarvol[[d]] <- sd(opvolsim[,d], na.rm=TRUE)^2
...
network_usl <- data.frame(matrix(NA, nrow=maxgates, ncol=maxlaterals))
rownames(network_usl) <- paste0("G", 1:5)
colnames(network_usl) <- paste0("L", 1:15)
network_usl[,] <- mean(flowdfl[[1]]$H)

network_g <- data.frame(matrix(NA, nrow=maxgates, ncol=maxlaterals))
rownames(network_g) <- paste0("G", 1:5)
colnames(network_g) <- paste0("L", 1:15)
network_g[,] <- mean(flowdfl[[1]]$G)


netopflowsim <- data.frame(matrix(NA, nrow=nsim, ncol=ngates))
...
for(l in 1:maxlaterals){ # need to talk to Nadya here!!!!!!!
  for(g in 1:maxgates){
    # operational error
    coefofvar <- 0.5
    astdv <- coefofvar*modelcoef[[d]][[5]]["A"]
    ...
    # send a wave, let's say it arrives equally at all gates
    network_usl[g,l]  
    
    # open gates at the same time and at different levels
    network_g[g,l]
    
    # uncertainty in operational flow
    for(i in 1:nsim){
      netopflowsim[i,d] <- (anorm[i]*gnorm[i]^2 + bnorm[i]*gnorm[i] + cnorm[i])*(hnorm[i]^ffnorm[i])
    }
    netopvarflow[[d]] <- sd(opflowsim[,d], na.rm=TRUE)^2
  }
}
