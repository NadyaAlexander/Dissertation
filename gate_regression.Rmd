...
montecarlosim <- function(coefofvar, dataindex){
  ...
  optvarflow <- optvarflowp*mean(flowsim)^2 # check this with Nadya, isn't this a better way than fixing the 6% at the mean, min and max?
...
for(d in 1:ngates){
  optresult[[d]] <- optim(par=c(1), fn=montecarlosim, dataindex=d, method="BFGS", control=list(maxit=10000, abstol=0.01 ))
}
```
