...
# 3.0 Model Stats
```{r model_stats}
library(hydroGOF) # remember to change bR2 equation, because error in hydroGOF package
...
operationalerrorsim <- function(dataindex, coefofvar=0.01, optflowerrorp=0.06){
  d <- dataindex
  # we can add the ability to call different functions here, but for now just equation #5
  atrue <- modelcoef[[d]][[5]]["A"] # assume the mean is the true value
  astdv <- coefofvar*modelcoef[[d]][[5]]["A"]
  ...
  gunif <- runif(nsim, min(flowdfl[[d]]$G), max(flowdfl[[d]]$G))
  hunif <- runif(nsim, min(flowdfl[[d]]$H), max(flowdfl[[d]]$H))
  tunif <- runif(nsim, 0, 1) # in hours, ask Nadya
  
  # now assign an error to each sampled G and H, added t for volume calcs
  tnorm <- gnorm <- hnorm <- vector()  
  for(i in 1:nsim){
    gnorm[i] <- rnorm(1, gunif[i],  1)
    hnorm[i] <- rnorm(1, hunif[i], 0.1)
    tnorm[i] <- rnorm(1, tunif[i], 0.25) # 15 minute standard deviation
  }
...
optresult <- replicate(n=ngates, expr=list())
for(d in 1:ngates){
  optresult[[d]] <- operationalerrorsim(dataindex=d, coefofvar=0.01, optflowerrorp=0.06)
}
```
...
# 8.0 Uncertainty at the Lateral Level - Laterals in Series
```{r lateral_network}
