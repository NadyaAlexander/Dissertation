...
for(i in 1:maxlaterals){
  for(j in 1:maxgates){ # this 0.01 value will have to be tweaked after running this a few times. for now just adding the normalized standard deviation to the gate standard deviations
    network_hsd_gcid_case1[j,i] <- 0.01*network_numgates_gcid[j,i] + sd(latlvl[latlvl$LATID=="48-1", "VALUE"])/mean(latlvl[latlvl$LATID=="48-1", "VALUE"])
    network_hsd_gcid_case2[j,i] <- 0.01*network_numgates_gcid[j,i] + sd(latlvl[latlvl$LATID=="22-1", "VALUE"])/mean(latlvl[latlvl$LATID=="22-1", "VALUE"])
  }
}
...
network_disaggregated_flow(networkresult_gcid_case1A, "1", "A")
network_disaggregated_flow(networkresult_gcid_case2A, "2", "A")
network_disaggregated_flow(networkresult_gcid_case1B, "1", "B")
network_disaggregated_flow(networkresult_gcid_case2B, "2", "B")
```
