...
# see where the nonlinearity is coming from
# log transformations to make it linear 

# or is the non-linearinty coming from the distributions

# evaluate function on a grid
cvrange <- seq(0.01, 1.5, 0.001)
optresult <- data.frame(matrix(NA, nrow=length(cvrange), ncol=ngates))
optresult$CV <- cvrange

for(d in 1:ngates){
  for(c in 1:length(cvrange)){
    cv <- cvrange[c]
    optresult[c, ngates] <- montecarlosim(coefofvar=cv, dataindex=d)
  }
}

png('outputs/optimization_function.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(optresult$CV, optresult$X4, xlab="Coefficient of Variation", ylab="Uncertainty Due to Errors in Flow Tables", pch=19, col="steelblue")
  # points(optresult$CV, optresult$X1, pch=19, col="steelblue1")
  # points(optresult$CV, optresult$X2, pch=19, col="steelblue2")
  # points(optresult$CV, optresult$X3, pch=19, col="steelblue3")
