...
flowdf[[1]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE, fileEncoding="UTF-8-BOM")
flowdf[[2]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE, fileEncoding="UTF-8-BOM")
flowdf[[3]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE, fileEncoding="UTF-8-BOM")
flowdf[[4]] <- read.csv("inputs/armco_gate18.csv", header = TRUE, check.names = FALSE, fileEncoding="UTF-8-BOM")
...
# 2.0 Fitting Gate Flow Equations
...
# Don't need this chunk, only needed to diagnose when the nls function fails
...
modelsim <- nls(FLOW~(A*G^2+B*G+C)*(H^FF), data=flowdfl[[1]], start=list(A=0, B=0.5, C=2.5, FF=0.5))
...
# model formulas, use this becuase nls can handle non linear modeling!
formulastring <- list()
formulastring[[1]] <- "FLOW ~(A*G^2+B*G+C)*(H^(D*G^2+E*G+FF))"
formulastring[[2]] <- "FLOW ~(B*G+C)*(H^(D*G^2+E*G+FF))"
formulastring[[3]] <- "FLOW ~(A*G^2+B*G+C)*(H^(E*G+FF))"
formulastring[[4]] <- "FLOW ~(C)*(H^(D*G^2+E*G+FF))"
formulastring[[5]] <- "FLOW ~(A*G^2+B*G+C)*(H^(FF))"
formulastring[[6]] <- "FLOW ~(C)*(H^(E*G+FF))"
formulastring[[7]] <- "FLOW ~(B*G+C)*(H^FF)"
formulastring[[8]] <- "FLOW ~(G)*log(H^FF)"
nformulas <- length(formulastring)

# term starts:    attempt No.1    2     3    4    5    6    7    8          
termstart <- data.frame( A=c(0,   NA,   0,   NA,  0,   NA,  NA,  NA), 
                         B=c(0.5, 0.5,  0.5, NA,  0,   NA,  0.5, NA), 
                         C=c(2,   1,    2.5, 1,   2.5, 1,   1,   NA), 
                         D=c(0,   0,    NA,  0,   NA,  NA,  NA,  NA), 
                         E=c(0,   0,    0,   0,   NA,  0,   NA,  NA), 
                        FF=c(0.5, 0.5,  0.5, 0.5, 0.5, 0.5, 0.5, 0.5))
```

```{r simfunction} 
# optim library? 
modelsim <- function(data, formulastring, termstart){
  nls(as.formula(formulastring), data=data, start=termstart) # change here!!!
}
...
# 3.0 Gate Flow Fitting Performance
...
# modified this for formulastrings that are in regular format not log format!
for(dd in 1:ngates){
  for (m in 1:nformulas){
    png(paste0("outputs/obspreplots/ovp_d", dd, "_m", m, ".png"), width=3.25, height=2.85, units="in", pointsize=8, res=1200)
      plot(flowdfl[[dd]]$FLOW, modelpred[[dd]][[m]], ylab="Observed Flow", xlab="Predicted Flow", pch=19)
      
      # add line for the perfect fit
      abline(0,1, col="grey80", lty=2, lwd=2)
      
      # add line for linear fit
      lmmod <- lm(modelpred[[dd]][[m]]~flowdfl[[dd]]$FLOW)
...
# note that optflowerrorp is the percentage 6 or 10 % error that is permissable. This error is related to the coefofvar allowed in the equation parameters. The largest allowed for 100% compliance with the 6% error is 0.02, and for 10% error is 0.05. 
...
png('outputs/operational_error/optimization_cloud.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4,2,1)+0.1)
  plot(operationresult[[1]][, "FLOWERROR"], operationresult[[1]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult[[1]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
  points(operationresult[[2]][, "FLOWERROR"], operationresult[[2]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult[[2]][, "ERRORCONDITION"]+1], pch=19)
  points(operationresult[[3]][, "FLOWERROR"], operationresult[[3]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult[[3]][, "ERRORCONDITION"]+1], pch=19)
  points(operationresult[[4]][, "FLOWERROR"], operationresult[[4]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult[[4]][, "ERRORCONDITION"]+1], pch=19)
...
png('outputs/operational_volume_error/optimization_cloud.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4,2,1)+0.1)
  plot(operationresult[[1]][, "VOLERROR"], operationresult[[1]][, "VOLTRUTH"], col=c("red", "steelblue1")[operationresult[[1]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Volume (cf)", ylab="True Volume (cf)")
  points(operationresult[[2]][, "VOLERROR"], operationresult[[2]][, "VOLTRUTH"], col=c("red", "steelblue2")[operationresult[[2]][, "ERRORCONDITION"]+1], pch=19)
  points(operationresult[[3]][, "VOLERROR"], operationresult[[3]][, "VOLTRUTH"], col=c("red", "steelblue3")[operationresult[[3]][, "ERRORCONDITION"]+1], pch=19)
  points(operationresult[[4]][, "VOLERROR"], operationresult[[4]][, "VOLTRUTH"], col=c("red", "steelblue")[operationresult[[4]][, "ERRORCONDITION"]+1], pch=19)
  mtext(paste0("Coefficient of Variation of Equation Parameters: 0.01"), side=3, line=0)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()
```

## 6.2 Sensitivity Analysis -- uncertaintiy with varying parameters one at a time
```{r uncertainty3}
# sa_par is the parameter of interest in the sensitivity analysis, valid values are: "ALL" for all coefficients having error at the same time; "A"; "B"; "C"; "FF"; "NONE", "NONE" is for cases where you want to change "G", "H", "t". We can do these ones from the equation inputs gsd, hsd, and tsd. 

operationalerrorsim_sa <- function(dataindex, coefofvar=0.01, optflowerrorp=0.06, gsd=0, hsd=0, tsd=0, sa_par){
  dd <- dataindex
  # we can add the ability to call different functions here, but for now just equation #5
  # for now just use the equation coefficients that Nadya got from excel
  atrue <- -0.07
  btrue <- 6.5
  ctrue <- 	0.8
  fftrue <- 0.4988
  ...
  #atrue <- modelcoef[[dd]][[5]]["A"] # assume the mean is the true value
  astdv <- abs(coefofvar*atrue)
  anorm <- rnorm(nsim, atrue, astdv) # samples around true value, creating error
  #btrue <- modelcoef[[dd]][[5]]["B"]
  bstdv <- coefofvar*btrue
  bnorm <- rnorm(nsim, btrue, bstdv)
  #ctrue <- modelcoef[[dd]][[5]]["C"]
  cstdv <- coefofvar*ctrue
  cnorm <- rnorm(nsim, ctrue, cstdv)
  #fftrue <- modelcoef[[dd]][[5]]["FF"]
  ffstdv <- coefofvar*fftrue
  ffnorm <- rnorm(nsim, fftrue, ffstdv)
  ...
  for(i in 1:nsim){
    ...
    if(sa_par=="A"){
      flowsim[i] <- (anorm[i]*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)
      flowtruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)
      volsim[i] <- (anorm[i]*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)*tunif[i]
      voltruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)*tunif[i]
    }
    if(sa_par=="B"){
      flowsim[i] <-  (atrue*gunif[i]^2 + bnorm[i]*gunif[i] + ctrue)*(hunif[i]^fftrue)
      flowtruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)
      volsim[i] <- (atrue*gunif[i]^2 + bnorm[i]*gunif[i] + ctrue)*(hunif[i]^fftrue)*tunif[i]
      voltruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)*tunif[i]
    }
    if(sa_par=="C"){
      flowsim[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + cnorm[i])*(hunif[i]^fftrue)
      flowtruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)
      volsim[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + cnorm[i])*(hunif[i]^fftrue)*tunif[i]
      voltruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)*tunif[i]
    }
    if(sa_par=="FF"){
      flowsim[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^ffnorm[i])
      flowtruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)
      volsim[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^ffnorm[i])*tunif[i]
      voltruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)*tunif[i]
    }
    if(sa_par=="NONE"){
      flowsim[i] <- (atrue*gnorm[i]^2 + btrue*gnorm[i] + ctrue)*(hnorm[i]^fftrue)
      flowtruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)
      volsim[i] <- (atrue*gnorm[i]^2 + btrue*gnorm[i] + ctrue)*(hnorm[i]^fftrue)*tnorm[i]
      voltruth[i] <- (atrue*gunif[i]^2 + btrue*gunif[i] + ctrue)*(hunif[i]^fftrue)*tunif[i]
    }
  }
  flowerror <- flowtruth-flowsim
...
cvrange <- seq(0.01, 0.5, 0.01)
operationresult_sa_all <- operationresult_sa_a <- operationresult_sa_b <- operationresult_sa_c <- operationresult_sa_ff <- operationresult_sa_g <- operationresult_sa_h <- operationresult_sa_t <- replicate(n=ngates, expr=list())
for(dd in 1:ngates){
  for(c in 1:length(cvrange)){
    cv <- cvrange[c]
    operationresult_sa_all[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=cv, optflowerrorp=0.06, gsd=0, hsd=0, tsd=0, sa_par="ALL")
    operationresult_sa_a[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=cv, optflowerrorp=0.06, gsd=0, hsd=0, tsd=0, sa_par="A")
    operationresult_sa_b[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=cv, optflowerrorp=0.06, gsd=0, hsd=0, tsd=0, sa_par="B")
    operationresult_sa_c[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=cv, optflowerrorp=0.06, gsd=0, hsd=0, tsd=0, sa_par="C")
    operationresult_sa_ff[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=cv, optflowerrorp=0.06, gsd=0, hsd=0, tsd=0, sa_par="FF")
    operationresult_sa_g[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=0, optflowerrorp=0.06, gsd=cv, hsd=0, tsd=0, sa_par="NONE")
    operationresult_sa_h[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=0, optflowerrorp=0.06, gsd=0, hsd=cv, tsd=0, sa_par="NONE")
    operationresult_sa_t[[dd]][[c]] <- operationalerrorsim_sa(dataindex=dd, coefofvar=0, optflowerrorp=0.06, gsd=0, hsd=0, tsd=cv, sa_par="NONE")
  }
}

# plotting
for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/ALL/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_all[[1]][[c]][, "FLOWERROR"], operationresult_sa_all[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_all[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_all[[2]][[c]][, "FLOWERROR"], operationresult_sa_all[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_all[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_all[[3]][[c]][, "FLOWERROR"], operationresult_sa_all[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_all[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_all[[4]][[c]][, "FLOWERROR"], operationresult_sa_all[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_all[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Coefficient of Variation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}
...
for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/A/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_a[[1]][[c]][, "FLOWERROR"], operationresult_sa_a[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_a[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_a[[2]][[c]][, "FLOWERROR"], operationresult_sa_a[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_a[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_a[[3]][[c]][, "FLOWERROR"], operationresult_sa_a[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_a[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_a[[4]][[c]][, "FLOWERROR"], operationresult_sa_a[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_a[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Coefficient of Variation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}
...
for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/B/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_b[[1]][[c]][, "FLOWERROR"], operationresult_sa_b[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_b[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_b[[2]][[c]][, "FLOWERROR"], operationresult_sa_b[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_b[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_b[[3]][[c]][, "FLOWERROR"], operationresult_sa_b[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_b[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_b[[4]][[c]][, "FLOWERROR"], operationresult_sa_b[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_b[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Coefficient of Variation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}
...
for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/C/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_c[[1]][[c]][, "FLOWERROR"], operationresult_sa_c[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_c[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_c[[2]][[c]][, "FLOWERROR"], operationresult_sa_c[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_c[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_c[[3]][[c]][, "FLOWERROR"], operationresult_sa_c[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_c[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_c[[4]][[c]][, "FLOWERROR"], operationresult_sa_c[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_c[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Coefficient of Variation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}
...
for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/FF/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_ff[[1]][[c]][, "FLOWERROR"], operationresult_sa_ff[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_ff[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_ff[[2]][[c]][, "FLOWERROR"], operationresult_sa_ff[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_ff[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_ff[[3]][[c]][, "FLOWERROR"], operationresult_sa_ff[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_ff[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_ff[[4]][[c]][, "FLOWERROR"], operationresult_sa_ff[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_ff[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Coefficient of Variation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}
...
for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/G/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_g[[1]][[c]][, "FLOWERROR"], operationresult_sa_g[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_g[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_g[[2]][[c]][, "FLOWERROR"], operationresult_sa_g[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_g[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_g[[3]][[c]][, "FLOWERROR"], operationresult_sa_g[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_g[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_g[[4]][[c]][, "FLOWERROR"], operationresult_sa_g[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_g[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Standard Deviation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}

for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/H/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_h[[1]][[c]][, "FLOWERROR"], operationresult_sa_h[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_h[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_h[[2]][[c]][, "FLOWERROR"], operationresult_sa_h[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_h[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_h[[3]][[c]][, "FLOWERROR"], operationresult_sa_h[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_h[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_h[[4]][[c]][, "FLOWERROR"], operationresult_sa_h[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_h[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Standard Deviation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}
...
for(c in 1:length(cvrange)){
  png(paste0('outputs/operational_error_sensitivity_analysis/t/optimization_cloud_', c, ".png"), width=3.25, height=3, units="in", pointsize=8, res=1200)
    par(mar=c(4,4,2,1)+0.1)
    plot(operationresult_sa_t[[1]][[c]][, "FLOWERROR"], operationresult_sa_t[[1]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue1")[operationresult_sa_t[[1]][[c]][, "ERRORCONDITION"]+1], pch=19, xlab="Error in Flow (cfs)", ylab="True Flow(cfs)")
    points(operationresult_sa_t[[2]][[c]][, "FLOWERROR"], operationresult_sa_t[[2]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue2")[operationresult_sa_t[[2]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_t[[3]][[c]][, "FLOWERROR"], operationresult_sa_t[[3]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue3")[operationresult_sa_t[[3]][[c]][, "ERRORCONDITION"]+1], pch=19)
    points(operationresult_sa_t[[4]][[c]][, "FLOWERROR"], operationresult_sa_t[[4]][[c]][, "FLOWTRUTH"], col=c("red", "steelblue")[operationresult_sa_t[[4]][[c]][, "ERRORCONDITION"]+1], pch=19)
    mtext(paste0("Standard Deviation: ", cvrange[c]), side=3, line=0)
    legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
  dev.off()
}
...
# second set of plots
png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytoALL.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_all[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Coefficient of Variation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_all[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_all[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_all[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytoA.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_a[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Coefficient of Variation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_a[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_a[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_a[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytoB.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_b[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Coefficient of Variation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_b[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_b[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_b[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytoC.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_c[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Coefficient of Variation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_c[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_c[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_c[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytoFF.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_ff[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Coefficient of Variation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_ff[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_ff[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_ff[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytoG.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_g[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Standard Deviation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_g[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_g[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_g[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytoH.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_h[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Standard Deviation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_h[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_h[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_h[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

png('outputs/operational_error_sensitivity_analysis/operational_error_sensitvitytot.png', width=3.25, height=3, units="in", pointsize=8, res=1200)
  par(mar=c(4,4.5,1,1)+0.1)
  plot(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_t[[1]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], xlab="Standard Deviation", ylab="No. of errors that exceed the 6% cutoff", pch=19, col="steelblue1")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_t[[2]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue2")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_t[[3]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue3")
  points(cvrange, nsim-do.call(rbind,lapply(operationresult_sa_t[[4]], colSums, na.rm=TRUE))[, "ERRORCONDITION"], pch=19, col="steelblue")
  # points(optresult[which.min(optresult$X4), "CV"], min(optresult$X4), pch=19, col="red")
  # text(paste0("min=(", optresult[which.min(optresult$X4), "CV"], ",", round(min(optresult$X4), 0), ")"), x=1.5*optresult[which.min(optresult$X4), "CV"], y=min(optresult$X4), pos=3, cex=0.8)
  legend("topleft", c("Gate 1", "Gate 2", "Gate 3", "Gate 4"), pch=c(19,19,19,19), col=c("steelblue1", "steelblue2", "steelblue3", "steelblue"), inset=c(0.02,0.02), cex=0.8, bg="grey96")
dev.off()

# How to depict this in one graph?

# writing to csv 
operationresults_to_csv <- function(listtoflatten){
  operationresultdf <- data.frame()
  for (i in 1:ngates){
    list_flat <- data.frame(do.call("rbind", listtoflatten[[i]]))
    list_flat$CV_SD <- rep(cvrange, 1, each=100)
    list_flat$NGATES <- i
    operationresultdf <- rbind(operationresultdf, list_flat)
  }
  write.csv(operationresultdf, paste0("outputs/operational_error_sensitivity_analysis/", deparse(substitute(listtoflatten)), ".csv"), row.names = FALSE)
}

operationresults_to_csv(operationresult_sa_all)
operationresults_to_csv(operationresult_sa_a)
operationresults_to_csv(operationresult_sa_b)
operationresults_to_csv(operationresult_sa_c)
operationresults_to_csv(operationresult_sa_ff)
operationresults_to_csv(operationresult_sa_g)
operationresults_to_csv(operationresult_sa_h)
operationresults_to_csv(operationresult_sa_t)
```
...
for(l in 1:maxlaterals){
  for(g in 1:maxgates){
    ...
    networkresult[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.02, optflowerrorp=0.06, gsd=0.1, hsd=hsdn, tsd=0.25)
  }
}
...
# make the uncertainty of the upstream level a function of the number of gates above the gate in the network, and include the uncertainty of a well behaved lateral vs. non-well behaved lateral.
...
for(i in 1:maxlaterals){
  for(j in 1:maxgates){ # this 0.01 value will have to be tweaked after running this a few times. for now just adding the normalized standard deviation to the gate standard deviations. 
...
# construct a run for each case, and each scenario. scenario A is for 6% error (0.02 coefofvar) and scenario B is for 10% error (0.05 coefoffar)
...
for(l in 1:maxlaterals){
  for(g in 1:maxgates){
    ...
    networkresult_gcid_case1A[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.02, optflowerrorp=0.06, gsd=0.1, hsd=hsdn, tsd=0.25)
    # # CASE II (not well-behaved lateral), SCENARIO A (accurate)
    # hsdn <- network_hsd_gcid_case2[g, l]
    # networkresult_gcid_case2A[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.02, optflowerrorp=0.06, gsd=0.1, hsd=hsdn, tsd=0.25)
    # # CASE I (well-behaved lateral), SCENARIO B (less accurate)
    # hsdn <- network_hsd_gcid_case1[g, l]
    # networkresult_gcid_case1B[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.05, optflowerrorp=0.10, gsd=0.1, hsd=hsdn, tsd=0.25)
    # # CASE II (not well-behaved lateral), SCENARIO B (less accurate)
    # hsdn <- network_hsd_gcid_case2[g, l]
    # networkresult_gcid_case2B[[l]][[g]] <- operationalerrorsim(dataindex=dd, coefofvar=0.05, optflowerrorp=0.10, gsd=0.1, hsd=hsdn, tsd=0.25)
  }
}
```
...
```{r gate_upstreamlvl_uncertainty_gcid_cases_tbd}
# Now that flow through the lateral and into the gates are disaggregated, calcualte the upstream level standard deviation properly.

# network_hsd_gcid_case1 <- network_hsd_gcid_case2 <- network_gatetype_gcid
# network_hsd_gcid_case1[,] <- NA
# 
# library(reshape2)
# network_disaggregated_flow_1A_wide <- dcast(network_disaggregated_flow_1A, GATENUM ~ LATID, value.var="FLOW_PER_GATE")
...
network_postprocessing2 <- function(data, casenum, scenario, alpha1, alpha2, beta1, beta2){
  ...
  png(paste0('outputs/network_alpha_beta_runs/gcid_mean_flowerror_C', casenum, 'S', scenario, "A1", alpha1, "_A2", alpha2, "_B1", beta1, "_B2", beta2, '.png'), width=20, height=8, units="in", pointsize=8, res=300)
    plot(gg1)
  dev.off()
  ...
  png(paste0('outputs/network_alpha_beta_runs/gcid_min_flowerror_C', casenum, 'S', scenario, "A1", alpha1, "_A2", alpha2, "_B1", beta1, "_B2", beta2, '.png'), width=20, height=8, units="in", pointsize=8, res=300) 
    plot(gg2)
  dev.off()
  ...
  png(paste0('outputs/network_alpha_beta_runs/gcid_max_flowerror_C', casenum, 'S', scenario, "A1", alpha1, "_A2", alpha2, "_B1", beta1, "_B2", beta2, '.png'), width=20, height=8, units="in", pointsize=8, res=300)
    plot(gg3)
  dev.off()
  ...
  png(paste0('outputs/network_alpha_beta_runs/gcid_mean_noncompliancepercentage_flowerror_C', casenum, 'S', scenario, "A1", alpha1, "_A2", alpha2, "_B1", beta1, "_B2", beta2, '.png'), width=20, height=8, units="in", pointsize=8, res=300)
    plot(gg4)
  dev.off()
}
