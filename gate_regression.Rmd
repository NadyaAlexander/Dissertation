...
for(i in 1:maxlaterals){
  for(j in 1:maxgates){ # this 0.01 value will have to be tweaked after running this a few times. for now just adding the normalized standard deviation to the gate standard deviations. 
    ...
    network_hsd_gcid_case1[j,i] <- 0.01*network_numgates_gcid[j,i] + sd(latlvl[latlvl$LATID=="48-1", "VALUE"])/mean(latlvl[latlvl$LATID=="48-1", "VALUE"])*1
    network_hsd_gcid_case2[j,i] <- 0.01*network_numgates_gcid[j,i] + sd(latlvl[latlvl$LATID=="22-1", "VALUE"])/mean(latlvl[latlvl$LATID=="22-1", "VALUE"])*1
  }
}
...
networkoperationalerrorsim <- function(alpha1=0.01, alpha2=0.01, beta1=1, beta2=1){
  for(i in 1:maxlaterals){
    for(j in 1:maxgates){ 
      network_hsd_gcid_case1[j,i] <- alpha1*network_numgates_gcid[j,i] + sd(latlvl[latlvl$LATID=="48-1", "VALUE"])/mean(latlvl[latlvl$LATID=="48-1", "VALUE"])*beta1
      network_hsd_gcid_case2[j,i] <- alpha2*network_numgates_gcid[j,i] + sd(latlvl[latlvl$LATID=="22-1", "VALUE"])/mean(latlvl[latlvl$LATID=="22-1", "VALUE"])*beta2
    }
  }
