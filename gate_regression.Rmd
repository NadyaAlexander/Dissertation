...
# plot percentage of times that the error requirement of 6% is not met
```
...
gcid_results$LATNUM <- rep(1:maxlaterals, times=1, each=maxgates*nsim)
gcid_results$GATENUM <- rep(1:maxgates, times=maxlaterals, each=nsim)
...
# plot min, max, mean, ... 
library(ggplot2)
png('outputs/mean_gcid_flowerror.png', width=20, height=8, units="in", pointsize=8, res=1200)
...
png('outputs/min_gcid_flowerror.png', width=20, height=8, units="in", pointsize=8, res=1200) 
...
png('outputs/max_gcid_flowerror.png', width=20, height=8, units="in", pointsize=8, res=1200)
...
networkresult_gcid <- replicate(n=2, expr=list())
for(l in 1:2){
  for(g in 1:3){
    dd <- network_gatetype_gcid[g, l]
    hsdn <- network_hsd_gcid[g, l]
...
dfresult_gcid$LATNUM <- rep(1:2, times=1, each=300)
dfresult_gcid$GATENUM <- rep(1:3, times=2, each=100)
dfresult_gcid$LATID <- colnames(network_gatetype_gcid[1:3, 1:2])[dfresult_gcid$LATNUM]
dfresult_gcid$GATEID <- rownames(network_gatetype_gcid[1:3, 1:2])[dfresult_gcid$GATENUM]
```
