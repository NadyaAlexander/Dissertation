...
```{r}
#IS THe BASE DATA COMING FROM FLOWdfl not LATLVL 
#WHERE TO GET THE SD 
#THE MODEL IS COMING FROM
#HAVE THE STANDARD DEVIATION FOR THE LEVEL BE A VARIABLE 
...
#Drawing on the Distribution 
Gmean<-mean(flowdfl$G)
Gsd<-sd(flowdfl$G)
Hmean<-mean(flowdfl$H)
Hsd<-sd(flowdfl$H)
...
#Reading the data and assigning the flow per gate
lateralflow <- read.csv("inputs/lateral_acre_flow.csv")
lateralflow$WATER_DELIVERED_AF

#Number of Simulations 
nsim<-1000

#Take the mean, the head has to be 20% larger, anything lower would be tossed. 
#Have a mean from
#Have a second mean to draw.

for (j in 1:length(lateralflow$LATID)){
  #Save the "lateral_acre_flow" column G as the "target flow" for each lateral.
  total_water_delivered<-lateralflow$WATER_DELIVERED_AF[j]
  #Randomize the "t" variable, have it be a number between 6 and 12.
  #choose a combination of g, h, and t from the probability distribution.
  gDraw <- rnorm(1,mean=Gmean,sd=Gsd)
  hDraw <- rnorm(1,mean=Hmean,sd=Hsd)
  tDraw <- runif(1, 6, 12)
  ...
  flowtruth <-vol<- vector()
  totalModeledFlow<-0
  i<-1
   while(totalModeledFlow<total_water_delivered[j]){
    # we can adj the ability to call different functions here, but for now just equation #1
    flowtruth[i] <- ( atrue + btrue * guniform[i] + ctrue * guniform[i] ^ 2 + dtrue * guniform[i] ^ 3 + etrue * log( huniform[i] ) ) / ( 1 + aatrue * guniform[i] +  bbtrue * log( huniform[i] ) + cctrue * ( log( huniform[i] ) ) ^ 2 + ddtrue * ( log( huniform[i] ) ) ^ 3 ) 
    
    
    vol[i] <- flowtruth[i]* tuniform[i]
    totalModeledFlow<-totalModeledFlow+vol[i]
    i = i+1
    print(totalModeledFlow)
   }
  hsd<-sd(huniform[1:i])
  gsd<-sd(guniform[1:i])
  tsd<-sd(tuniform[1:i])
