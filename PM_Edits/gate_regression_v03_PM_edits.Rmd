...
png(paste0('outputs/gcid_section7.png'), width=20, height=8, units="in", pointsize=8, res=300)#<----need to set the chart name
    plot(section7chart)
    print(section7chart)
```
...
#Set the number of Monte Carlo Runs
nsim <- 1000 #<------Set the number of Monte Carlo Runs for drawing errors
nruns<-100  #<------Set the number of total loops to run 
#Sum of Squares Function
ssd <- function(x) sqrt(sum(x*x))  #<------How sd's are combined 
...
#From observation there are a maximum of 72 gates for a single lateral.
maxgates <- 72 #<-----set the maximum number of gates
maxlaterals <- nrow(gcid)
...
#Read in Data from LatLVL
latlvl <- read.csv("inputs/lateral_levels_longformat.csv")
network_hsd_gcid_case1 <- network_hsd_gcid_case2 <- network_gatetype_gcid
network_hsd_gcid_case1[,] <- network_hsd_gcid_case2[,] <- NA

#Calculate the initial HSD based on how far down the lateral 
  for(j in 1:maxgates){ # this 0.01 value will have to be tweaked after running this a few times. for now just adjing the normalized standard deviation to the gate standard deviations. 
    #Combine the two standard deviations using sdt = sqrt(sd1^2 + sd2^2)
    network_hsd_gcid_case1[j,i] <- ((0.01*network_numgates_gcid[j,i])^2 + (sd(latlvl[latlvl$LATID=="48-1", "VALUE"])/mean(latlvl[latlvl$LATID=="48-1", "VALUE"])*1)^2)^0.5
    network_hsd_gcid_case2[j,i] <- ((0.01*network_numgates_gcid[j,i])^2 + (sd(latlvl[latlvl$LATID=="22-1", "VALUE"])/mean(latlvl[latlvl$LATID=="22-1", "VALUE"])*1)^2)^0.5
...
#Set Up Per Gate Water Limits in the needed structre
#Need to deal with subgates, right now assuming that they are cumulative
network_volumepergate_gcid<-data.frame(network_numgates_gcid, check.names = F)
GateNames<-colnames(network_volumepergate_gcid)
GateNames[1:176]<-gsub("([[:alpha:]])","",colnames(network_volumepergate_gcid)[1:176])
GateNames[1:176]<-substr(GateNames[1:176], 1, 5)
...
#Number of Upstream Gates
gatedata_long$upstreamGates<-gcid$UPSTREAM_GATE_COUNT[match(gatedata_long$LATID,gcid$LATID)]
...
gatedata_long$USL_sd_ngates[i]<-ssd(c(.2,rep(.2,gatedata_long$UpstreamGates[i])))
gatedata_long$UCL_sd_ngates[i]<-ssd(c(.5,rep(.5,gatedata_long$UpstreamGates[i])))
}

sectionEightGateDataUSL<-list()
sectionEightGateDataUCL<-list()
for (i in 1:nrow(gatedata_long)){
sectionEightGateDataUCL[[i]]<-operationalerrorsim(dataindex=di, coefofvar=0.05, optflowerrorp=0.10, gsd=0.1, hsd=gatedata_long$UCL_sd_ngates[i], tsd=0.25)
sectionEightGateDataUSL[[i]]<-operationalerrorsim(dataindex=di, coefofvar=0.05, optflowerrorp=0.10, gsd=0.1, hsd=gatedata_long$USL_sd_ngates[i], tsd=0.25)
#print(i)
}
