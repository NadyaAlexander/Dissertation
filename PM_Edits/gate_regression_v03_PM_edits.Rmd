...
knitr::opts_chunk$set(echo = TRUE, eval=TRUE, warning = FALSE,tidy.opts=list(width.cutoff=80), tidy=TRUE)
setwd("/Users/TheVault/Desktop/Upwork/Dissertation/PM_Edits/") #<-----set the working drive
```
...
flowdf <- read.csv("/Users/TheVault/Desktop/Upwork/Dissertation/inputs/armco_gate18.csv", header = TRUE, check.names = FALSE, fileEncoding="UTF-8-BOM") #PM Comment: Reads in data, header checks sets the first 
...
#PM Comment: removes NA cells from flow data frame (na.omit), takes column names and shifts them into
# rows (gather), changes G to a number instead of a string(mutate)
...
cvrange <- seq(0.01, 0.5, 0.01) #<-----set the CV range 
optresult <-list()
...
Glist<-seq(0,1,1/30) #<-----set up a sequence of values to loop over for gsd, right now we from zero to 1, in increments of 1/30 
...
ggsave(filename ="gPlot.png", #Save the file with the some options
       ...
       path="outputs/sensitivity_analysis_pm/", #<------Path to directory where it should be saved
...
Hlist<-seq(0,1,1/30)#<-----set up a sequence of values to loop over for hsd, right now we from zero to 1, in increments of 1/30 
...
ggsave(filename ="hPlot.png",
       plot=H_plot,
       path="outputs/sensitivity_analysis_pm/",
      device="png",
      width=6.5,
      height=5, 
      units="in")
...
#T Sd
plotList_Tsd<-list()
Tlist<-seq(0,1,1/30)#<-----set up a sequence of values to loop over for tsd, right now we from zero to 1, in increments of 1/30 
...
#CV
plotList_cv<-list()
cvlist<-seq(0,1,1/30)#<-----set up a sequence of values to loop over for cv, right now we from zero to 1, in increments of 1/30 
...
dev.off()
  ...
  ######Code for R Markdown Display, Can be Deleted
  plot6a<-ggplot(toymod_abs_mean_flowerror, aes(x=LATNUM, y=GATENUM, size=MEANABSFLOWERROR, fill=MEANABSFLOWERROR)) +
    geom_point(shape = 21)+
    scale_fill_distiller(palette = "Spectral")+
    labs(fill = "Mean \nAbsolute \nFlow Error \n(cfs)", size="Mean \nAbsolute \nFlow Error \n(cfs)")+
    xlab("Lateral No.") + 
...
dev.off()
  ...
  ######Code for R Markdown Display, Can be Deleted
  plot6b<-ggplot(toymod_min_flowerror, aes(x=LATNUM, y=GATENUM, size=MINABSFLOWERROR, fill=MINABSFLOWERROR)) +
    geom_point(shape = 21)+
    scale_fill_distiller(palette = "Spectral")+
    labs(fill = "Minimum \nAbsolute \nFlow Error \n(cfs)", size="Minimum \nAbsolute \nFlow Error \n(cfs)")+
    xlab("Lateral No.") + 
...
dev.off()
  ...
  ######Code for R Markdown Display, Can be Deleted
  plot6c<-ggplot(toymod_max_flowerror, aes(x=LATNUM, y=GATENUM, size=MAXABSFLOWERROR, fill=MAXABSFLOWERROR)) +
    geom_point(shape = 21)+
    scale_fill_distiller(palette = "Spectral")+
    labs(fill = "Maximum \nAbsolute \nFlow Error \n(cfs)", size="Maximum \nAbsolute \nFlow Error \n(cfs)")+
    xlab("Lateral No.") + 
...
dev.off()
 ...
 ######Code for R Markdown Display, Can be Deleted
  plot6d<-ggplot(toymod_abs_mean_errorcondition, aes(x=LATNUM, y=GATENUM, size=ERRORCONDITIONNOTMET, fill=ERRORCONDITIONNOTMET)) +
    geom_point(shape = 21)+
    scale_fill_distiller(palette = "Spectral")+
    labs(fill = "Percentage of \nNon-Compliance \n(N=100)", size="Percentage of \nNon-Compliance \n(N=100)")+
    xlab("Lateral No.") + 
...
dev.off()
 ...
 ######Code for R Markdown Display, Can be Deleted
  plot6_3a<-ggplot(latlvl[latlvl$LATID=="48-1", ], aes(VALUE)) +
  geom_density(alpha=0.5, position = "stack") +
  geom_rug(aes(x = VALUE, y = 0), position = position_jitter(height = 0)) +
  xlab("Lateral Levels") +
  ylab("Density")
...
dev.off()
  ...
  ######Code for R Markdown Display, Can be Deleted
  plot6_3b<-ggplot(latlvl[latlvl$LATID=="22-1", ], aes(VALUE)) +
  geom_density(alpha=0.5, position = "stack") +
  geom_rug(aes(x = VALUE, y = 0), position = position_jitter(height = 0)) +
  xlab("Lateral Levels") +
  ylab("Density")
...
dev.off()
  ...
  ######Code for R Markdown Display, Can be Deleted
  plot6_3c<-ggplot(latlvl[latlvl$LATID=="21-4", ], aes(VALUE)) +
  geom_density(alpha=0.5, position = "stack") +
  geom_rug(aes(x = VALUE, y = 0), position = position_jitter(height = 0)) +
  xlab("Lateral Levels") +
  ylab("Density")
...
#Set the number of Monte Carlo Runs
nsim <- 1000 #<------Set the number of Monte Carlo Runs for drawing errors
nruns<-10  #<------Set the number of total loops to run 
...
# Creating a cumulative vector for each lateral, this shows the relative position of each gate from the origin
network_numgates_gcid <- network_gatetype_gcid
network_numgates_gcid[,] <- NA
for(i in 1:maxlaterals){
  for(j in 1:gcid[i,"NUM_GATES_COL_COUNT"]){
...
##Get actual data from LatLVL
latlvl <- read.csv("inputs/lateral_levels_longformat.csv")
network_hsd_gcid_case1 <- network_hsd_gcid_case2 <- network_gatetype_gcid
network_hsd_gcid_case1[,] <- network_hsd_gcid_case2[,] <- NA


#Calculate the HSD based on gate position relative to origin and case based on real data
# this 0.01 value will have to be tweaked after running this a few times. for now just adjing the normalized standard deviation to the gate standard deviations. 

sdIncrease<-.01 #<------set the sd increase amount
case1<-"48-1"#<------select the lateral data to draw case 1 from
case2<-"22-1"#<------select the lateral data to draw case 2 from

for(i in 1:maxlaterals){
  for(j in 1:maxgates){ 
    #Combine the two standard deviations using sdt = sqrt(sd1^2 + sd2^2)
    network_hsd_gcid_case1[j,i] <- ((sdIncrease*network_numgates_gcid[j,i])^2 + (sd(latlvl[latlvl$LATID==case1, "VALUE"])/mean(latlvl[latlvl$LATID==case1, "VALUE"])*1)^2)^0.5
    network_hsd_gcid_case2[j,i] <- ((sdIncrease*network_numgates_gcid[j,i])^2 + (sd(latlvl[latlvl$LATID==case2, "VALUE"])/mean(latlvl[latlvl$LATID==case2, "VALUE"])*1)^2)^0.5
  }
}

#Setting up a function based on the total volume, this runs the loop only up to the point where we get "a total volume for that gate"
volumeerrorsim <- function(dataindex, coefofvar=0.02, optflowerrorp=0.06, gsd=0.1, hsd=0.1, tsd=0.25,totalvolume){
...
#Reading the data and calculating the average flow per gate
lateralflow <- read.csv("inputs/lateral_acre_flow.csv")
totalGatesGCID<-gcid%>%group_by(PARENT)%>%summarise(NUM_GATES_COL_COUNT=sum(NUM_GATES_COL_COUNT))
colnames(totalGatesGCID)[1]<-"LATID"
lateralflow <- merge(lateralflow, totalGatesGCID[, c("LATID", "NUM_GATES_COL_COUNT")], by="LATID",all=T)
lateralflow$FLOW_PER_GATE <- lateralflow$WATER_DELIVERED_AF/lateralflow$NUM_GATES_COL_COUNT

#Set up per Gate water limits in the needed structure
network_volumepergate_gcid<-data.frame(network_numgates_gcid, check.names = F)
#Change gate names to parent gate name
...
for(i in 1:ncol(network_volumepergate_gcid)){
  ...
  network_volumepergate_gcid[!is.na(network_volumepergate_gcid[,i]),i]<-lateralflow$FLOW_PER_GATE[grep(GateNames[i],lateralflow$LATID)]}  

#Create a main gate data
#NumGates
...
for(h in 1:nruns){
  for(j in 1:nrow(gatedata_long_ml)){
    ...
    totalvolumen<-gatedata_long_ml$FLOW_PER_GATE[j]
     result<-data.frame(volumeerrorsim(dataindex=di, coefofvar=0.02, optflowerrorp=0.06, gsd=0.1, hsd=hsdn, tsd=0.25, totalvolume = totalvolumen))
...
#Combine results using the sqrt of the sum of squares for the sds and mean flow error
plotData<-rbindlist(network_all)
plotData_final<-plotData%>%group_by(LATID,GATENUM)%>%summarise(FLOWERROR=mean(abs(FLOWERROR),na.rm=T),
                                                               VOLTRUTH=mean(VOLTRUTH,na.rm=T),
                                            hsd=sqrt(sum(hSDvec^2)),
                                            tsd=sqrt(sum(tSDvec^2)),
                                            gsd=sqrt(sum(gSDvec^2)))

write.csv(plotData_final,'outputs/gcid_section7.csv')#<----need to set the table save name

#Plot the results n
library(ggplot2)
section7chart<-ggplot(plotData_final, aes(x=LATID, y=GATENUM, size=FLOWERROR, fill=FLOWERROR))+ 
    geom_point(shape = 21)+
    scale_fill_distiller(palette = "Spectral")+
    labs(fill = "Mean \nAbsolute \nFlow Error \n(cfs)", size="Mean \nAbsolute \nFlow Error \n(cfs)")+
    xlab("Lateral No.") + 
    ylab("Gate No.")+ theme(axis.text.x = element_text(angle = 90, hjust = 1))


png(paste0('outputs/gcid_section7.png'), width=20, height=8, units="in", pointsize=8, res=300)#<----need to set the chart save name
    plot(section7chart)
    print(section7chart)
```


#8 New Section Experimenting with Error Propegation
...
gatedata_long$USL_sd_ngates[i]<-ssd(c(USL_initial,(USL_sdIncrease*gatedata_long$GATENUM[i])))
gatedata_long$UCL_sd_ngates[i]<-ssd(c(UCL_initial,(UCL_sdIncrease*gatedata_long$GATENUM[i])))
}
...
for (i in 1:nrow(gatedata_long)){
resultUSL<-data.frame(operationalerrorsim(dataindex=di, coefofvar=0.05, optflowerrorp=0.10, gsd=0.1, hsd=gatedata_long$USL_sd_ngates[i], tsd=0.25))
    resultUSL$LATID<-gatedata_long$LATID[i]
    resultUSL$GATENUM<-gatedata_long$GATENUM[i]

sectionEightGateDataUSL[[i]]<-resultUSL

resultUCL<-data.frame(operationalerrorsim(dataindex=di, coefofvar=0.05, optflowerrorp=0.10, gsd=0.1, hsd=gatedata_long$UCL_sd_ngates[i], tsd=0.25))
      resultUCL$LATID<-gatedata_long$LATID[i]
    resultUCL$GATENUM<-gatedata_long$GATENUM[i]
    
sectionEightGateDataUCL[[i]]<-resultUCL

#print(i)
}

#Combine results using the sqrt of the sum of squares for the sds and mean flow error
plotDataUSL<-rbindlist(sectionEightGateDataUSL)
plotDataUCL<-rbindlist(sectionEightGateDataUCL)

plotData_finalUSL<-plotDataUSL%>%group_by(LATID,GATENUM)%>%summarise(FLOWERROR=mean(abs(FLOWERROR),na.rm=T),
                                                               VOLTRUTH=mean(VOLTRUTH,na.rm=T))
plotData_finalUCL<-plotDataUCL%>%group_by(LATID,GATENUM)%>%summarise(FLOWERROR=mean(abs(FLOWERROR),na.rm=T),
                                                               VOLTRUTH=mean(VOLTRUTH,na.rm=T))

write.csv(plotData_finalUSL,'outputs/gcid_section8USL.csv')#<----need to set the table save name
write.csv(plotData_finalUCL,'outputs/gcid_section8UCL.csv')#<----need to set the table save name

#Plot the results 
section8chartUSL<-ggplot(plotData_finalUSL, aes(x=LATID, y=GATENUM, size=FLOWERROR, fill=FLOWERROR))+ 
    geom_point(shape = 21)+
    scale_fill_distiller(palette = "Spectral")+
    labs(fill = "Mean \nAbsolute \nFlow Error \n(cfs)", size="Mean \nAbsolute \nFlow Error \n(cfs)")+
    xlab("Lateral No.") + 
...
section8chartUCL<-ggplot(plotData_finalUCL, aes(x=LATID, y=GATENUM, size=FLOWERROR, fill=FLOWERROR))+ 
    geom_point(shape = 21)+
    scale_fill_distiller(palette = "Spectral")+
    labs(fill = "Mean \nAbsolute \nFlow Error \n(cfs)", size="Mean \nAbsolute \nFlow Error \n(cfs)")+
    xlab("Lateral No.") + 
...
png(paste0('outputs/gcid_section8USL.png'), width=20, height=8, units="in", pointsize=8, res=300)#<----need to set the chart save name
    section8chartUSL
    dev.off()
    
    png(paste0('outputs/gcid_section8UCL.png'), width=20, height=8, units="in", pointsize=8, res=300)#<----need to set the chart save name
    section8chartUCL
    dev.off()
...
```{r all_netwrok_model_by+alpha_beta, eval=FALSE}
